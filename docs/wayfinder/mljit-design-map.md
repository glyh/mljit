...
<!-- MILESTONE (2026-08-09): module-level compilation lands, closing the
cross-function-call gap. One ExecBuffer per module, every function emitted into it
in declaration order, calls lowered as `call rel32` to the callee's label — so
mutual recursion resolves through the assembler's existing forward fixups. The last
remaining v1 emission gap is i1-as-a-value. -->
...
- [Implement Module-Level Compilation (Cross-Function Calls)](tickets/implement-module-level-compilation.md) — shipped the module-level driver exactly as designed, with **no new machinery in `mljit.x64`**: `codegen::compile(mod, fid) -> ExecBuffer` became `compile(mod) -> CompiledModule` (one buffer + a `FunctionId -> byte offset` table, `invoke<T>(fid, args...)`), one `Label` per function allocated up front, per-function emission moved to an internal `emit_function`, one `finalize()`. A call site is `a.call(func_label[callee])`, so **mutual recursion works through the ordinary forward fixup**. `ExecBuffer` gained `invoke_at<T>(offset, ...)` so every function of a module is independently callable. The CLI's `jit_gap` scan became **whole-module** (honest, since compilation is eager and whole-module) with function-qualified messages, and its cross-function arm is retired. Suite 139 → 144: mutual recursion, a three-function call chain, and a mixed-frame (spill-heavy caller + recursive `fib`) module, all differential against the interpreter.
